...
replay_window_sets = sa.Table(
    "replay_window_sets",
    metadata,
    sa.Column("window_set_id", sa.String(96), primary_key=True),
    ...
    sa.Column("setup_types_json", sa.JSON, nullable=False, server_default=sa.text("'[]'::jsonb")),
    sa.Column("start", sa.DateTime(timezone=True)),
    sa.Column("end", sa.DateTime(timezone=True)),
    sa.Column("window_mode", sa.String(32), nullable=False),
    sa.Column("window_size_days", sa.Integer),
    sa.Column("step_days", sa.Integer),
    sa.Column("embargo_minutes", sa.Integer),
    sa.Column("session", sa.String(32), nullable=False, server_default="rth"),
    sa.Column("replay_config_json", sa.JSON, nullable=False, server_default=sa.text("'{}'::jsonb")),
    sa.Column("sensitivity_config_json", sa.JSON, nullable=False, server_default=sa.text("'{}'::jsonb")),
    sa.Column("validation_config_json", sa.JSON, nullable=False, server_default=sa.text("'{}'::jsonb")),
    sa.Column("summary_json", sa.JSON, nullable=False, server_default=sa.text("'{}'::jsonb")),
...
replay_window_results = sa.Table(
    "replay_window_results",
    metadata,
    sa.Column("window_result_id", sa.String(96), primary_key=True),
    sa.Column("window_set_id", sa.String(96), nullable=False, index=True),
    ...
    sa.Column("test_start", sa.DateTime(timezone=True)),
    sa.Column("test_end", sa.DateTime(timezone=True)),
    sa.Column("replay_start", sa.DateTime(timezone=True)),
    sa.Column("replay_end", sa.DateTime(timezone=True)),
    sa.Column("replay_run_ids_json", sa.JSON, nullable=False, server_default=sa.text("'[]'::jsonb")),
    sa.Column("counterfactual_replay_run_id", sa.String(96)),
    sa.Column("portfolio_replay_run_id", sa.String(96)),
...
model_calibration_drift_reports = sa.Table(
    "model_calibration_drift_reports",
    metadata,
    sa.Column("drift_report_id", sa.String(96), primary_key=True),
    sa.Column("model_version", sa.String(128), nullable=False, index=True),
    sa.Column("calibration_audit_ids_json", sa.JSON, nullable=False, server_default=sa.text("'[]'::jsonb")),
    sa.Column("window_result_ids_json", sa.JSON, nullable=False, server_default=sa.text("'[]'::jsonb")),
    sa.Column("replay_run_ids_json", sa.JSON, nullable=False, server_default=sa.text("'[]'::jsonb")),
    sa.Column("summary_json", sa.JSON, nullable=False, server_default=sa.text("'{}'::jsonb")),
    sa.Column("score_bin_drift_json", sa.JSON, nullable=False, server_default=sa.text("'{}'::jsonb")),
    sa.Column("grade_bin_drift_json", sa.JSON, nullable=False, server_default=sa.text("'{}'::jsonb")),
    sa.Column("action_bin_drift_json", sa.JSON, nullable=False, server_default=sa.text("'{}'::jsonb")),
    sa.Column("stability_metrics_json", sa.JSON, nullable=False, server_default=sa.text("'{}'::jsonb")),
    sa.Column("drift_flags_json", sa.JSON, nullable=False, server_default=sa.text("'[]'::jsonb")),
...
model_calibration_drift_windows = sa.Table(
    "model_calibration_drift_windows",
    metadata,
    sa.Column("id", sa.String(96), primary_key=True),
    sa.Column("drift_report_id", sa.String(96), nullable=False, index=True),
    sa.Column("window_result_id", sa.String(96)),
    sa.Column("window_index", sa.Integer, nullable=False),
    sa.Column("metrics_json", sa.JSON, nullable=False, server_default=sa.text("'{}'::jsonb")),
    sa.Column("flags_json", sa.JSON, nullable=False, server_default=sa.text("'[]'::jsonb")),
    sa.Column("severity", sa.String(32), nullable=False),
    sa.Column("created_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.func.now()),
)

model_review_reports = sa.Table(
    "model_review_reports",
    metadata,
    sa.Column("review_report_id", sa.String(96), primary_key=True),
    sa.Column("model_version", sa.String(128), nullable=False, index=True),
    sa.Column("window_set_id", sa.String(96)),
    sa.Column("validation_report_ids_json", sa.JSON, nullable=False, server_default=sa.text("'[]'::jsonb")),
    sa.Column("calibration_audit_ids_json", sa.JSON, nullable=False, server_default=sa.text("'[]'::jsonb")),
    ...
    sa.Column("readiness_reasons_json", sa.JSON, nullable=False, server_default=sa.text("'[]'::jsonb")),
    sa.Column("unresolved_warnings_json", sa.JSON, nullable=False, server_default=sa.text("'[]'::jsonb")),
    sa.Column("payload_json", sa.JSON, nullable=False),
    sa.Column("created_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.func.now()),
)

research_cycles = sa.Table(
    "research_cycles",
    metadata,
    sa.Column("id", sa.Integer, primary_key=True, autoincrement=True),
    sa.Column("research_cycle_id", sa.String(96), nullable=False, unique=True, index=True),
    sa.Column("cycle_date", sa.Date, nullable=False, index=True),
...
research_cycle_artifacts = sa.Table(
    "research_cycle_artifacts",
    metadata,
    sa.Column("cycle_artifact_id", sa.String(96), primary_key=True),
    sa.Column("research_cycle_id", sa.String(96), nullable=False, index=True),
...
champion_challenger_comparisons = sa.Table(
...
model_proposals = sa.Table(
    "model_proposals",
    metadata,
    sa.Column("id", sa.Integer, primary_key=True, autoincrement=True),
    sa.Column("proposal_id", sa.String(96), nullable=False, unique=True, index=True),
...
model_decision_ledger = sa.Table(
    "model_decision_ledger",
    metadata,
    sa.Column("id", sa.Integer, primary_key=True, autoincrement=True),
    sa.Column("decision_id", sa.String(96), nullable=False, unique=True, index=True),
    sa.Column("decision_type", sa.String(64), nullable=False, index=True),
    sa.Column("research_cycle_id", sa.String(96), index=True),
    sa.Column("proposal_id", sa.String(96), index=True),
    sa.Column("model_version", sa.String(128), index=True),
...
provider_capability_checks = sa.Table(
    ...
    sa.Column("operator_review_status", sa.String(32), nullable=False, server_default="UNREVIEWED", index=True),
...
quote_snapshots = sa.Table(
    "quote_snapshots",
    metadata,
    sa.Column("id", sa.Integer, primary_key=True, autoincrement=True),
    sa.Column("quote_snapshot_id", sa.String(96), nullable=False, unique=True, index=True),
    sa.Column("provider", sa.String(32), nullable=False, index=True),
    sa.Column("endpoint_key", sa.String(64), nullable=False, index=True),
    sa.Column("symbol", sa.String(16), nullable=False, index=True),
    sa.Column("timestamp_utc", sa.DateTime(timezone=True), nullable=False, index=True),
    sa.Column("provider_timestamp", sa.String(64)),
    ...
    sa.Column("source", sa.String(32), nullable=False, server_default="fmp"),
    sa.Column("ingestion_run_id", sa.String(96)),
    sa.Column("provider_request_id", sa.String(96)),
    sa.Column("raw_fields_json", sa.JSON, nullable=False, server_default=sa.text("'{}'::jsonb")),
    sa.Column("data_quality_flags_json", sa.JSON, nullable=False, server_default=sa.text("'[]'::jsonb")),
    sa.Column("created_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.func.now()),
    sa.UniqueConstraint("provider", "endpoint_key", "symbol", "timestamp_utc", name="uq_quote_snapshots_provider_symbol_ts"),
)

data_freshness_reports = sa.Table(
    "data_freshness_reports",
    metadata,
    sa.Column("id", sa.Integer, primary_key=True, autoincrement=True),
    sa.Column("freshness_report_id", sa.String(96), nullable=False, unique=True, index=True),
    sa.Column("provider", sa.String(32), nullable=False, index=True),
    sa.Column("status", sa.String(32), nullable=False, index=True),
    sa.Column("symbols_json", sa.JSON, nullable=False, server_default=sa.text("'[]'::jsonb")),
    sa.Column("intervals_json", sa.JSON, nullable=False, server_default=sa.text("'[]'::jsonb")),
    sa.Column("required_capability_endpoints_json", sa.JSON, nullable=False, server_default=sa.text("'[]'::jsonb")),
    sa.Column("latest_bars_json", sa.JSON, nullable=False, server_default=sa.text("'[]'::jsonb")),
    sa.Column("latest_quotes_json", sa.JSON, nullable=False, server_default=sa.text("'[]'::jsonb")),
    sa.Column("missing_items_json", sa.JSON, nullable=False, server_default=sa.text("'[]'::jsonb")),
    sa.Column("stale_items_json", sa.JSON, nullable=False, server_default=sa.text("'[]'::jsonb")),
    sa.Column("dirty_windows_json", sa.JSON, nullable=False, server_default=sa.text("'[]'::jsonb")),
    sa.Column("capability_summary_json", sa.JSON, nullable=False, server_default=sa.text("'{}'::jsonb")),
    sa.Column("warnings_json", sa.JSON, nullable=False, server_default=sa.text("'[]'::jsonb")),
    sa.Column("recommendations_json", sa.JSON, nullable=False, server_default=sa.text("'[]'::jsonb")),
    sa.Column("max_bar_age_minutes_json", sa.JSON, nullable=False, server_default=sa.text("'{}'::jsonb")),
    sa.Column("max_quote_age_seconds", sa.Integer),
